...
import { FiX, FiDroplet, FiSun, FiThermometer, FiCheck, FiAlertTriangle, FiBookOpen } from "react-icons/fi";
...
interface CareGuideDetailModalProps {
  plant: PlantGuide | null;
  onClose: () => void;
}

export default function CareGuideDetailModal({ plant, onClose }: CareGuideDetailModalProps) {
  useEffect(() => {
    if (plant) {
  ...
  const currentDiff = plant ? (difficultyMeta[plant.difficulty] || difficultyMeta.Easy) : difficultyMeta.Easy;

  return (
    <AnimatePresence>
      {plant && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-8 overflow-y-auto">
          {/* Backdrop (Framer Motion Fades In) */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          {/* Premium Card Layout (Framer Motion Slides & Scales In) */}
          <motion.div
          ...
          >
            ...
            {/* Right Side: Detailed Scrollable Guide Content */}
            <div className="w-full md:w-1/2 p-6 sm:p-8 overflow-y-auto flex flex-col gap-6 scroll-smooth scrollbar-thin scrollbar-thumb-sage/20">
              ...
              {/* Core Care Instructions */}
              <div className="space-y-3.5 bg-white border border-sage/15 rounded-2xl p-5 shadow-sm">
                <div className="flex items-center gap-2 border-b border-sage/15 pb-2.5">
                  <FiBookOpen className="text-forest text-sm" />
                  <h3 className="text-xs font-black uppercase tracking-wider text-forest-dark">
                    Step-by-Step Care Steps
                  </h3>
                </div>
